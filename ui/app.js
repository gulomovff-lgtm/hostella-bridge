...
(() => {
  ...
  const note = (el, text, tone) => { el.textContent = text; el.className = `note ${tone || ''}`; el.hidden = !text; };
  ...
  const demoApi = () => {
    const state = {
      version: '0.3.1', profile: 'demo', host: 'STAND-PC', paired: true,
    ...
    return { getState: async () => state, pair: ok, unpair: ok, setCreds: ok, openPortal: async () => ({ ok: true, loggedIn: true }), testPortal: async () => ({ ok: true, loggedIn: true }), sweepNow: ok, setAutostart: ok, openLogs: ok, installUpdate: ok, quit: ok, onState: () => {} };
  };
  ...
  function render(s) {
    ...
    if (paired) {
      ...
      $('p-host').textContent = s.host || '—';
      const ses = s.session || {};
      let tone = 'ok', text = 'в сети', tip = 'Мост отмечается в кассе каждые 30 секунд; касса считает его в сети, пока молчание короче 90 секунд.';
      if (ses.starting) { tone = ''; text = 'подключаюсь…'; tip = ''; }
      else if (ses.reason === 'revoked') { tone = 'bad'; text = 'отключён администратором'; tip = 'В кассе нажали «Отключить мост». Чтобы продолжить, возьмите новый код и подключитесь заново.'; }
      else if (!ses.ok) { tone = 'warn'; text = ses.reason === 'offline' ? 'нет связи' : 'ошибка'; tip = ses.message || 'Проверьте интернет. Мост попробует снова через минуту.'; }
      pill($('pair-pill'), text, tone);
      $('p-session').textContent = text;
      $('p-note').textContent = tip;
      head = { text, tone };
    ...
    const needLogin = !!(w && w.paused === 'need_login');
    if (needLogin) pill($('portal-pill'), 'нужен вход', 'bad');
    else if (!s.portal.hasLogin) pill($('portal-pill'), 'логин не задан', 'warn');
    else pill($('portal-pill'), 'учётка сохранена', 'ok');
    $('btn-login').textContent = needLogin ? 'Войти в e-mehmon…' : 'Открыть портал e-mehmon';
    $('btn-login').disabled = !paired && !s.portal.hasLogin;
    ...
    // Защита данных — одной строкой; подробности в подсказке по наведению
    const sec = s.security || {};
    const secure = sec.httpsOnly !== false && sec.secretsEncrypted !== false;
    $('sec-text').textContent = secure ? 'Соединение защищено' : 'Соединение не защищено';
    $('sec-lock').className = `lock ${secure ? 'ok' : 'bad'}`;
    $('sec-lock').title = `Только HTTPS, ${sec.tlsMin || 'TLS 1.2'} и выше · ${(sec.hosts || []).join(', ') || '—'}\nПаспортные данные только в памяти, журнал без номеров паспортов\nПароль портала и ключ моста — под шифрованием Windows`;
  }
  ...
  $('btn-pair').addEventListener('click', async () => {
    ...
    try {
      ...
      if (!r.ok) note($('pair-err'), r.message || 'Не удалось подключиться', 'bad');
      else $('code').value = '';
    } finally {
  ...
  async function saveCreds() {
    ...
    credsDirty = false;
    $('password').value = '';
    note($('portal-note'), 'Учётка сохранена.', 'ok');
    refresh();
  }
  $('btn-creds').addEventListener('click', saveCreds);
  $('btn-login').addEventListener('click', async () => {
    if (credsDirty) await saveCreds();
    $('btn-login').disabled = true;
    try {
      const r = await api.openPortal();
      if (r && r.ok === false) note($('portal-note'), `Не удалось открыть портал: ${r.message || ''}`, 'bad');
      else if (r && r.loggedIn) note($('portal-note'), 'Сессия портала жива — окно открыто, входить не нужно.', 'ok');
      else note($('portal-note'), 'Открыта страница входа: подтвердите капчу и нажмите «Войти».', '');
    } finally {
  ...
  $('btn-test').addEventListener('click', async () => {
    ...
    try {
      ...
      note($('portal-note'), !r.ok ? `Не удалось: ${r.message}` : r.loggedIn ? 'Сессия портала жива — вход не нужен.' : 'Портал просит войти — нажмите «Войти в e-mehmon…».', !r.ok ? 'bad' : r.loggedIn ? 'ok' : 'bad');
    } finally {
